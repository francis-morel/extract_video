...
def openFile():
    ...
    start_time.set("0")
    end_time.set(math.trunc(cap.get(cv.CAP_PROP_FRAME_COUNT) / cap.get(cv.CAP_PROP_FPS)) + 1)
...
def runExtraction():
    ...
    is_time_valid, debut, fin = validateTimes(start_time.get(), end_time.get())
...
def extract():
    ...
    min, sec, ms = msec_to_minute_seconde_msec(round(cap.get(cv.CAP_PROP_POS_FRAMES) / cap.get(cv.CAP_PROP_FPS) * 1000))
    image_name = destination_directory + "/" + filename + "_" + min + "m_" + sec + "s_" + ms + "ms" + ".jpg"
    ...
    setWidgetWhileExtracting(in_progress)
    root.after(20, extract)

def msec_to_minute_seconde_msec(time):
    timeleft = time
...
def setWidgetWhileExtracting(isExtracting):
    if isExtracting:
...
# Window 2 components
ttk.Label(window_step2, text="Nom du fichier:").grid(row=0, column=0, sticky=W, **default_options)
info_file_name_variable = tk.StringVar()
ttk.Label(window_step2, textvariable=info_file_name_variable).grid(row=0, column=1, sticky=W, **default_options)

ttk.Label(window_step2, text="FPS:").grid(row=1, column=0, sticky=W, **default_options)
info_fps_variable = tk.StringVar()
ttk.Label(window_step2, textvariable=info_fps_variable).grid(row=1, column=1, sticky=W, **default_options)

ttk.Label(window_step2, text="Durée:").grid(row=2, column=0, sticky=W, **default_options)
info_duree_variable = tk.StringVar()
ttk.Label(window_step2, textvariable=info_duree_variable).grid(row=2, column=1, sticky=W, **default_options)

ttk.Label(window_step2, text="Temps de début (s):").grid(row=3, column=0, sticky=W, **default_options)
...
ttk.Label(window_step2, text="Temps de fin (s):").grid(row=4, column=0, sticky=W, **default_options)
...
start_extraction_button = ttk.Button(window_step2, text="Extraire", command=runExtraction)
start_extraction_button.grid(row=7, column=1, **default_options)

stop_extraction_button = ttk.Button(window_step2, text="Arrêter", command=stopExtraction)
